...
from utils.inc_net import MTKDCILFNet
...
class MTKDCILF(BaseLearner):
    def __init__(self, args):
        super().__init__(args)
        self.args = args
        self._network = MTKDCILFNet(args, False)
        self._basenet = None
    ...
    def _update_representation(self, train_loader, test_loader, optimizer, scheduler):
        ...
        ax.set_xticks(np.arange(0, self._total_classes+1, 10))
        ax.set_yticks(np.arange(0, self._total_classes+1, 10))

        ax.set_xticklabels(np.arange(0, self._total_classes+1, 10))
        ax.set_yticklabels(np.arange(0, self._total_classes+1, 10))


        ax.set_xlabel("Predicted labels")
        ax.set_ylabel("True labels")
        ax.set_title("Confusion Matrix")
        plt.savefig("iVirusshareDataset60-120_1000_confusion_matrix.png")
        plt.close()
    ...
    def _prune_network(self, train_loader, test_loader, optimizer, scheduler):
        ...
        # # 打印剪枝后的模型参数量
        # total_params = sum(p.numel() for p in self._network.parameters() if p.requires_grad)
        ...
        for i in range(torch.cuda.device_count()):
            mem = torch.cuda.memory_reserved(i) / 1024 / 1024 / 1024
            total_mem += mem
        info = f"after prune net the GPU memory allocated: {total_mem:.2f} GB"
     
        
        logging.info(info)
        sparsity = self.count_sparsity(self._network, p=False)
        info = f"after prune net the model sparsity is : {sparsity:.2f} "
        logging.info(info)
        ...
        ax.set_ylabel("True labels")
        ax.set_title("Confusion Matrix")
        plt.savefig("MY_iVirusshareYearDataset47-57_2000_confusion_matrix.png")
        plt.close()
