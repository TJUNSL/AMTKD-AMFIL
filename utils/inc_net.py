...
# from convs.resnext import resnext18, resnext34, resnext50, resnext101, resnext152
...
def get_convnet(args, pretrained=False):
    ...
    # elif name == "resnext18":
    #     return resnext18(args=args)
    # elif name == "resnext34":
    #     return resnext34(args=args)
    # elif name == "resnext50":
    #     return resnext50(args=args)
    # elif name == "resnext101":
    #     return resnext101(args=args)
    # elif name == "resnext152":
    #     return resnext152(args=args)
...
class IncrementalNetWithBias(BaseNet):
    def __init__(self, args, pretrained, bias_correction=False):
        ...
        # nn.ModuleList([])的作用是将所有的模块放到一个list中，这样可以方便的管理所有的参数，但并没有forward的功能，需要自己补充
...
class FOSTERNet(nn.Module):
    ...
    def forward(self, x):
        ...
        IB_features = Last_convnet(x)["IB_features"]
        
        features = torch.cat(features, 1)
        out = self.fc(features)
        fe_logits = self.fe_fc(features[:, -self.out_dim :])["logits"]

        out.update({"fe_logits": fe_logits, "features": features, "IB_features": IB_features})
...
class KINDROIDNet(nn.Module):
    def __init__(self, args, pretrained):
        super(KINDROIDNet, self).__init__()
        self.convnet_type = args["convnet_type"]
        self.convnets = nn.ModuleList()
        self.pretrained = pretrained
        self.out_dim = None
    ...
    def forward(self, x):
        ...
        out.update({ "features": features, "IB_features": IB_features})
